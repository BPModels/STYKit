...
NSString * const TYNetworkAccessibityChangedNotification = @"TYNetworkAccessibityChangedNotification";
...
@interface TYNetworkAccessibity_ty(){
    ...
    TYNetworkAccessibleState_ty _previousState;
...
+ (TYNetworkAccessibity_ty *)sharedInstance {
    static TYNetworkAccessibity_ty * instance = nil;
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(void (^)(TYNetworkAccessibleState_ty))block {
...
- (void)setupNetworkAccessibity {
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"TYNetworkAccessibityRunFlag_ty";
...
- (void)cleanNetworkAccessibity {
    ...
    _previousState = TYNetworkChecking_ty;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    TYNetworkAccessibity_ty *networkAccessibity = (__bridge TYNetworkAccessibity_ty *)info;
    if (![networkAccessibity isKindOfClass: [TYNetworkAccessibity_ty class]]) {
        return;
    }
...
- (void)startCheck {
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
        ...
        [self notiWithAccessibleState:TYNetworkAccessible_ty];
        return;
    }
        ...
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(ZYNetworkType type) {
                ...
                if (type == ZYNetworkTypeCellularData || type == ZYNetworkTypeWiFi) {
                    [self notiWithAccessibleState:TYNetworkRestricted_ty];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:TYNetworkUnknown_ty];
                }
            }];
...
- (void)notiWithAccessibleState:(TYNetworkAccessibleState_ty)state {
    if (_automaticallyAlert) {
        if (state == TYNetworkRestricted_ty) {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:TYNetworkAccessibityChangedNotification object:nil];
