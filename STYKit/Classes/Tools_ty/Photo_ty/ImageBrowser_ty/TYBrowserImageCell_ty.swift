...
class TYBrowserImageCell_ty: TYCollectionViewCell_ty, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    private var progressView_ty = TYProgressView_ty(type: .round, size: CGSize(width: AdaptSize_ty(60), height: AdaptSize_ty(60)), lineWidth: AdaptSize_ty(5))
    ...
    func setCustomData_ty(model: TYMediaImageModel_ty, userId: String) {
...
//            }
    ...
    private func getOriginImage_ty(model: TYMediaImageModel_ty, progress: ((CGFloat) ->Void)?, completion: ((UIImage?)->Void)?, userId: String) {
        if let image = model.image_ty {
            completion?(image)
            return
        }
        if let _sessionId = model.sessionId_ty, _sessionId.isNotEmpty_ty,
           let imageData = TYFileManager_ty.share_ty.getSessionMedia_ty(type: .sessionImage, name: model.originLocalPath_ty!, sessionId: _sessionId, userId: userId){
            // 如果是聊天室查看
            ...
            if let _path = model.originLocalPath_ty, let image = UIImage(contentsOfFile: _path) {
                ...
                TYDownloadManager_ty.share_ty.image_ty(urlStr: path, progress: progress, completion: completion)
            } else {
                completion?(nil)
            }
        }
