...
/// 声音管理器
@objc(TYPlayAudioManager)
open class TYPlayAudioManager: NSObject {

    @objc
    public static let share_ty = TYPlayAudioManager()
    ...
    public func playAudio_ty(url: URL, finishedBlock: DefaultBlock_ty?) {
        TYAuthorizationManager.share_ty.authorizeMicrophoneWith_ty { result in
            if result {
