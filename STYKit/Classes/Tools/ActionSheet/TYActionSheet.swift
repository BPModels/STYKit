...
open class TYActionSheet: TYTopWindowView {
    ...
    public func addItem_ty(icon: UIImage? = nil, title: String, isDestroy: Bool = false, actionBlock: DefaultBlock_ty?) -> TYActionSheet {
        let button = TYButton(.normal)
    ...
    @objc public func clickAction_ty(sender: TYButton) {
    ...
    public override func show_ty(view: UIView = kWindow_ty) {
        ...
        TYRecordAudioManager.share_ty.shake_ty()
        UIView.animate(withDuration: 0.25) { [weak self] in
            guard let self = self else { return }
