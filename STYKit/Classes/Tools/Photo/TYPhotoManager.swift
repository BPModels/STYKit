...
open class TYPhotoManager: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    @objc public static let share_ty = TYPhotoManager()
    /// 系统相机的拍照后的回调使用
    private var completeBlock_ty: (([TYMediaModel])->Void)?
    
    /// 显示选择相机还是相册
    /// - Parameters:
    ///   - block: 选择后的回调
    ///   - maxCount: 最大选择数量
    public func show_ty(complete block:(([TYMediaModel])->Void)?, maxCount: Int = 1) {
        TYActionSheet().addItem_ty(title: "相册", actionBlock: {
            TYPhotoManager.share_ty.showPhoto_ty(complete: { (modelList) in
                block?(modelList)
            }, maxCount: maxCount)
        }).addItem_ty(title: "相机", actionBlock: {
            TYPhotoManager.share_ty.showCamera_ty { (modelList) in
                block?(modelList)
            }
        }).show_ty()
    }
    ...
    public func showCamera_ty(complete block:(([TYMediaModel])->Void)?) {
        TYAuthorizationManager.share_ty.camera_ty { [weak self] (result) in
    ...
    ///   - block: 选择后的回调
    ///   - maxCount: 最大选择数量
    public func showPhoto_ty(complete block: (([TYMediaModel])->Void)?, maxCount: Int = 1) {
        TYAuthorizationManager.share_ty.photo_ty { (result) in
            guard result else { return }
            if UIImagePickerController.isSourceTypeAvailable(.photoLibrary) {
                let vc = TYSystemPhotoViewController()
