...
protocol TYBrowserVideoCellDelegate: NSObjectProtocol {
...
class TYBrowserVideoCell:
    TYCollectionViewCell,
    TYVideoManagerDelegate,
    UIGestureRecognizerDelegate {
    
    weak var delegate_ty: TYBrowserVideoCellDelegate?
    private let playManager_ty = TYVideoManager()
    ...
    private var customContentView_ty = TYView()
    
    private var playButton_ty: TYButton = {
        let button = TYButton()
    ...
    private var leftTimeLabel_ty: TYLabel = {
        let label = TYLabel()
        label.text          = "00:00"
    ...
    private var rightTimeLabel_ty: TYLabel = {
        let label = TYLabel()
        label.text          = "00:00"
        label.textColor     = UIColor.white
        label.font          = UIFont.custom_ty(.PingFangTCRegular, size: AdaptSize_ty(12))
    ...
    private var progressView_ty = TYProgressView(type: .line, size: CGSize(width: AdaptSize_ty(250), height: AdaptSize_ty(2)))
    
    private var closedButton_ty: TYButton = {
        let button = TYButton()
