...
public enum TYImageBrowserType {
    /// 自定义
    case custom_ty(modelList: [TYMediaModel])
    /// 系统相册
    case system_ty(result: [PHAsset])
}

public protocol TYImageBrowserDelegate: NSObjectProtocol {
    func reuploadImage(model:[TYMediaModel])
}

public class TYBrowserView:
    TYView,
    UICollectionViewDelegate,
    UICollectionViewDataSource,
    TYBrowserImageCellDelegate,
    TYBrowserVideoCellDelegate {
    
    public weak var delegate_ty:TYImageBrowserDelegate?
    
    private let kTYBrowserImageCellID_ty = "kTYBrowserImageCellID"
    private let kTYBrowserVideoCellID_ty = "kTYBrowserVideoCellID"
    private var medioModelList_ty: [TYMediaModel] = []
    private var assetModelList_ty: PHFetchResult<PHAsset>?
    private var type_ty: TYImageBrowserType
...
//        layout.minimumInteritemSpacing = AdaptSize_ty(20)
    ...
    private var albumButton_ty: TYButton = {
        let button = TYButton()
    ...
    public init(type: TYImageBrowserType, current index: Int) {
        self.type_ty         = type
...
//        TYChatRequestManager.share.requestRecord(content: "查看图片大图")
    ...
    public override func bindProperty_ty() {
        ...
        self.collectionView_ty.register(TYBrowserImageCell.classForCoder(), forCellWithReuseIdentifier: kTYBrowserImageCellID_ty)
        self.collectionView_ty.register(TYBrowserVideoCell.classForCoder(), forCellWithReuseIdentifier: kTYBrowserVideoCellID_ty)
    ...
    private func showAlubmVC_ty() {
        let vc = TYPhotoAlbumViewController()
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .custom_ty(let modelList):
            ...
            case .image_ty:
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kTYBrowserImageCellID_ty, for: indexPath) as? TYBrowserImageCell, let imageModel = model as? TYMediaImageModel else {
                    return TYCollectionViewCell()
                }
                cell.delegate_ty = self
                cell.setCustomData_ty(model: imageModel, userId: "")
                return cell
            case .video_ty:
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kTYBrowserVideoCellID_ty, for: indexPath) as? TYBrowserVideoCell, let videoModel = model as? TYMediaVideoModel else {
                    return TYCollectionViewCell()
                }
                ...
                return cell
            default:
                return TYBrowserImageCell()
            }
        case .system_ty(let result):
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kTYBrowserImageCellID_ty, for: indexPath) as? TYBrowserImageCell else {
                return TYCollectionViewCell()
            }
            cell.delegate_ty = self
    ...
    public func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        (cell as? TYBrowserImageCell)?.updateZoomScale_ty()
    }
    ...
    func videoViewLongPressAction_ty(model: TYMediaVideoModel?) {
        print("保存视频暂未适配")
    }
    ...
    public func longPressAction_ty(image: UIImage?) {
        TYActionSheet().addItem_ty(title: "保存") {
