...
protocol TYSystemAlbumListViewDelegate: NSObjectProtocol {
    func selectedAlbum_ty(model: TYPhotoAlbumModel?)
    func showAlbumAction_ty()
    func hideAlbumAction_ty()
}

class TYSystemAlbumListView: TYView, UITableViewDelegate, UITableViewDataSource {

    private let cellID_ty = "kBPSystemAlbumCell"
    private var albumList_ty: [TYPhotoAlbumModel] = []
    private var currentModel_ty: TYPhotoAlbumModel? {
        didSet {
    ...
    weak var delegate_ty: TYSystemAlbumListViewDelegate?
    ...
    var tableView_ty: TYTableView = {
        let tableView = TYTableView()
    ...
    override func bindProperty_ty() {
        ...
        self.tableView_ty.register(TYSystemAlbumCell.classForCoder(), forCellReuseIdentifier: cellID_ty)
    ...
    func setData_ty(albumList: [TYPhotoAlbumModel], current model: TYPhotoAlbumModel?) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellID_ty, for: indexPath) as? TYSystemAlbumCell else {
