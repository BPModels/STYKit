...
protocol TYPhotoAlbumCellDelegate: NSObjectProtocol {
...
class TYMediaCell: TYCollectionViewCell {
    /// 历史照片列表使用
    var model_ty: TYMediaModel?
    /// 系统相册列表使用
    var assetMode_ty: PHAsset?

    weak var delegate_ty: TYPhotoAlbumCellDelegate?

    private var disableShadowView_ty: TYView = {
        let view = TYView()
    ...
    private var iconImageView_ty: TYImageView = {
    ...
    private var selectButton_ty: TYButton = {
        let button = TYButton()
    ...
    /// 显示历史照片浏览
    func setData_ty(model: TYMediaModel, hideSelect: Bool, isSelected: Bool) {
        self.model_ty                   = model
...
//            if let videoModel = model as? TYMediaVideoModel {
...
//            }
        ...
        case .image_ty:
            ...
            self.timeLabel_ty.text = ""
//            if let imageModel = model as? TYMediaImageModel {
...
//            }
    ...
    @objc private func selectedImage(sender: TYButton) {
