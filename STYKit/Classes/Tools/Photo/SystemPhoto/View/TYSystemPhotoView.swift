...
protocol TYSystemPhotoViewDelegate: NSObjectProtocol {
    func clickImage_ty(indexPath: IndexPath, from imageView: UIImageView?)
    func selectedImage_ty()
    func unselectImage_ty()
}

class TYSystemPhotoView: TYView, UICollectionViewDelegate, UICollectionViewDataSource, TYPhotoAlbumCellDelegate {
    
    weak var delegate_ty: TYSystemPhotoViewDelegate?
    ...
    override func bindProperty_ty() {
        ...
        self.collectionView_ty.register(TYMediaCell.classForCoder(), forCellWithReuseIdentifier: self.kBPPhotoAlbumCellID_ty)
    }
    
    // MARK: ==== Event ====
    
    func reload(album model: TYPhotoAlbumModel?) {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kBPPhotoAlbumCellID_ty, for: indexPath) as? TYMediaCell, let asset = self.albumModel_ty?.assets_ty[indexPath.row] else {
