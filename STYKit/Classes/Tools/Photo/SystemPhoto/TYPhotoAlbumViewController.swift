//
//  TYPhotoAlbumViewController.swift
//  STYKit
//
...
public protocol TYPhotoAlbumViewControllerDelegate: TYPhotoAlbumToolsDelegate {}

/// 方格显示传递过来的资源
public class TYPhotoAlbumViewController:
    TYViewController,
    UICollectionViewDelegate,
    UICollectionViewDataSource,
    TYPhotoAlbumToolsDelegate,
    TYPhotoAlbumCellDelegate {
    
    public weak var delegate_ty: TYPhotoAlbumViewControllerDelegate?
    ...
    /// 总资源
    public var modelList_ty: [TYMediaModel] = []
    /// 已选资源
    var selectedList_ty: [TYMediaModel] = []
    ...
    private var toolsView_ty = TYPhotoAlbumToolsView()
    ...
    public override func bindProperty_ty() {
        ...
        self.collectionView_ty.register(TYMediaCell.classForCoder(), forCellWithReuseIdentifier: self.kTYPhotoAlbumCellID_ty)
    }
    ...
    // MARK: ==== TYPhotoAlbumToolsDelegate ====
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kTYPhotoAlbumCellID_ty, for: indexPath) as? TYMediaCell else {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let cell = collectionView.cellForItem(at: indexPath) as? TYMediaCell else {
            return
        }
        TYBrowserView(type: .custom_ty(modelList: modelList_ty), current: indexPath.row).show_ty(animationView: cell.imageView_ty)
    }

    // MARK: ==== TYPhotoAlbumCellDelegate ====
    func selectedImage_ty(model: Any) {
        guard let _model = model as? TYMediaModel, !self.selectedList_ty.contains(_model) else { return }
        self.selectedList_ty.append(_model)
        self.collectionView_ty.reloadData()
    }

    func unselectImage_ty(model: Any) {
        guard let _model = model as? TYMediaModel, let index = self.selectedList_ty.firstIndex(of: _model) else { return }
