...
/// 优先级由高到低
public enum TYAlertPriorityEnum: Int {
    case A = 0
    case B = 1
    case C = 2
    case D = 3
...
open class TYBaseAlertView: TYTopWindowView {
    
    /// 弹框优先级
    public var priority_ty: TYAlertPriorityEnum = .normal
    /// 是否已展示过
...
//    open var attributionView: KFAttributionView?
    ...
    /// 背景图
    internal var backgroundImage_ty: TYImageView = {
    ...
    /// 左边按钮
    open var leftButton_ty: TYButton = {
        let button = TYButton()
    ...
    /// 右边按钮
    open var rightButton_ty: TYButton = {
        let button = TYButton(.theme, animation: false)
        button.size_ty = CGSize(width: AdaptSize_ty(100), height: AdaptSize_ty(35))
